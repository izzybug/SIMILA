...
<?php
	...
	$result = mysqli_query($conn,"update tblemployees set FirstName='$fname', LastName='$lname', EmailId='$email', Gender='$gender', Dob='$dob', Department='$department', Dosbing='$dosbing', Address='$address', role='$user_role', NIM='$NIM' where emp_id='$get_id'         
		");
	if ($result) {
							...
							<section>
								...
								<div class="row">
									...
									<div class="col-md-4 col-sm-12">
										<div class="form-group">
											<label>NIM :</label>
											<input name="NIM" type="text" class="form-control" required="true" maxlength="15" autocomplete="off"value="<?php echo $row['NIM']; ?>">
										</div>
									</div>
								</div>
								<div class="row">
									...
									<div class="col-md-4 col-sm-12">
										<div class="form-group">
											...
											<select name="department" class="custom-select form-control" required="true" autocomplete="off">
												<?php
													$query_student = mysqli_query($conn,"select * from tblemployees join  tblprogramstudi where emp_id = '$get_id'")or die(mysqli_error());
								...
								<div class="row">
									<div class="col-md-4 col-sm-12">
										<div class="form-group">
											...
											<select name="dosbing" class="custom-select form-control" required="true" autocomplete="off">
												<?php
													$query = mysqli_query($conn,"select * from tbldosbing join tblemployees where emp_id = '$get_id'")or die(mysqli_error());
													$row = mysqli_fetch_array($query)
													
													?>
												<option value="<?php echo $row['Dosbing']; ?>"><?php echo $row['Dosbing']; ?></option>
													<?php
													$query = mysqli_query($conn,"select * from tbldosbing");
													while($row = mysqli_fetch_array($query)){
